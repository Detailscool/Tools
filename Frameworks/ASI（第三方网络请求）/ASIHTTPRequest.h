...
// !!!: ASIHTTPReqeust 本身继承自 NSOperation
...
#pragma mark get information about this request - 获取请求信息
...
/** 返回二进制数据，如果合适(gzip)，会自动解压缩 */
- (NSData *)responseData;

// Returns true if the response was gzip compressed
/** 如果支持 gzip 压缩，返回 true */
- (BOOL)isResponseCompressed;

#pragma mark running a request － 运行请求
...
#pragma mark upload/download progress － 上传/下载进度
...
#pragma mark talking to delegates － 通知代理工作
...
#pragma mark parsing HTTP response headers － 解析 HTTP 响应头
...
#pragma mark http authentication stuff － HTTP 身份验证
...
#pragma mark stream status handlers － “流”状态处理(Socket)
...
#pragma mark persistent connections － 持久化连接
...
#pragma mark keychain storage － 钥匙串存储
...
#pragma mark get user agent － 客户端类型
...
#pragma mark mime-type detection - mime-type 检测(上传文件要提供mimetype)

// Return the mime type for a file
+ (NSString *)mimeTypeForFileAtPath:(NSString *)path;

#pragma mark bandwidth measurement / throttling - 带宽测量 / 节流
...
#pragma mark reachability － 连接状态的检测
...
#pragma mark queue － 队列

// Returns the shared queue
// 返回一个共享的队列，在 ASI 底层异步是通过操作队列来实现的
...
#pragma mark network activity － 网络活动（小圈圈）
...
#pragma mark threading behaviour － 线程行为
...
@property (atomic, retain) NSError *error;                  // 网络访问出现错误
